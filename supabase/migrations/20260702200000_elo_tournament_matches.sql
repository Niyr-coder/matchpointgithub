-- 20260702200000 · ELO para bracket_matches y tournament_group_matches.
-- Extiende el sistema de mig 058 (fn_recalculate_elo_for_match + trigger en
-- matches casual) a los partidos de torneo. Mismo algoritmo K=32, mismo starting
-- rating 2500, misma idempotencia via rating_applied_at.
--
-- Chain sport para bracket_matches:
--   bracket_matches.bracket_id → brackets.tournament_id → tournaments.sport
-- Chain sport para tournament_group_matches:
--   tournament_group_matches.group_id → tournament_groups.category_id
--   → tournament_categories.tournament_id → tournaments.sport
...
comment on column bracket_matches.rating_applied_at is
  'Timestamp de aplicación de ELO. NULL = aún no aplicado. Idempotencia del trigger.';
comment on column tournament_group_matches.rating_applied_at is
  'Timestamp de aplicación de ELO. NULL = aún no aplicado. Idempotencia del trigger.';

-- ---------------------------------------------------------------------------
-- 2) Función para bracket_matches
-- ---------------------------------------------------------------------------
...
comment on function fn_recalculate_elo_for_bracket_match(uuid) is
  'Recalcula ELO (K=32) para un bracket_match. Idempotente via rating_applied_at. '
  'Sport resuelto via brackets → tournaments. Player IDs vía registrations.';

-- ---------------------------------------------------------------------------
-- 3) Función para tournament_group_matches
-- ---------------------------------------------------------------------------
...
comment on function fn_recalculate_elo_for_group_match(uuid) is
  'Recalcula ELO (K=32) para un tournament_group_match. Idempotente via rating_applied_at. '
  'Sport resuelto via tournament_groups → tournament_categories → tournaments.';
...
-- 5) Triggers
-- ---------------------------------------------------------------------------
drop trigger if exists tg_elo_on_bracket_match_report on bracket_matches;
create trigger tg_elo_on_bracket_match_report
  after update of winner_side, status on bracket_matches
  for each row
  execute function tg_bracket_matches_elo_on_update();

drop trigger if exists tg_elo_on_group_match_report on tournament_group_matches;
create trigger tg_elo_on_group_match_report
  after update of winner_side, status on tournament_group_matches
  for each row
  execute function tg_group_matches_elo_on_update();
