...
function Heatmap({ heatmap }: { heatmap: number[][] }) {
  ...
  return <div className="card" style={{ padding: "20px 22px" }}><div className="flex items-center justify-between mb-3.5"><div><span className="label-mp">Cuándo juega</span><div className="card-title">Heatmap semanal</div></div><div className="flex items-center gap-1">{palette.map((c) => <span key={c} style={{ width: 9, height: 9, borderRadius: 2, background: c, border: `1px solid ${tk.border}` }} />)}</div></div><div className="mp-table-scroll"><div style={{ minWidth: 280, display: "grid", gridTemplateColumns: "20px repeat(6, 1fr)", gap: 3, alignItems: "center" }}><div />{HOURS.map((hour) => <div key={hour} style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 8.5, color: tk.muted, textAlign: "center" }}>{hour}</div>)}{heatmap.map((row, di) => <div key={DAYS[di]} style={{ display: "contents" }}><div className="font-heading" style={{ fontWeight: 900, fontSize: 11, textAlign: "center" }}>{DAYS[di]}</div>{row.map((v, fi) => <div key={`${DAYS[di]}-${fi}`} style={{ aspectRatio: "1.4/1", borderRadius: 3, background: palette[v], border: v === 0 ? `1px solid ${tk.border}` : "1px solid transparent" }} />)}</div>)}</div></div></div>;
}
