...
// Cliente: timeline de auditoría filtrada por evento. Llama a la action
// admin-only `getEventAuditLog` (audit_log + event_registrations + transactions
// del evento) y renderiza una línea cronológica con diff JSON expandible.
...
const ENTITY_COLOR: Record<string, string> = {
  events: "#0ea5e9",
  event_registrations: "var(--primary)",
  tournaments: "#0ea5e9",
...
export function EventAuditLog({ eventId }: { eventId: string }) {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const res = await getEventAuditLog({ eventId, limit: 50 });
      if (cancelled) return;
