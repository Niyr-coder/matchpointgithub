...
import { signInFromForm, signUpFromForm, beginGoogleOAuthFromForm } from "@/server/actions/auth";
...
function SignUpForm({ next, onSwitch }: { next?: string; onSwitch: () => void }) {
  ...
  return (
    <>
      <GoogleOAuthSection intent="signup" next={next} />
...
function SignInForm({ next, onSwitch }: { next?: string; onSwitch: () => void }) {
  ...
  return (
    <>
      <GoogleOAuthSection intent="signin" next={next} />
...
function GoogleOAuthSection({
  ...
  const [state, action, pending] = useActionState(beginGoogleOAuthFromForm, null);
