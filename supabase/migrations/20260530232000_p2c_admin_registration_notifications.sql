-- P2-C · Notificaciones para cambios admin en inscripciones de eventos/torneos.
-- Agrega kinds faltantes y recrea el dispatcher in-app con branches legibles.

insert into notification_kinds (kind, description, allowed_roles, default_channels, category) values
  ('event_registration_cancelled', 'Inscripción a evento cancelada por administración',
   array['user']::mp_role[], array['inapp']::mp_notification_channel[], 'events'),
  ('event_registration_transferred', 'Cupo de evento transferido por administración',
   array['user']::mp_role[], array['inapp']::mp_notification_channel[], 'events'),
  ('event_registration_no_show', 'No-show marcado en evento por administración',
   array['user']::mp_role[], array['inapp']::mp_notification_channel[], 'events'),
  ('tournament_registration_removed', 'Inscripción a torneo removida por administración',
   array['user']::mp_role[], array['inapp']::mp_notification_channel[], 'tournaments')
