import { createResource, listResources } from "@/server/actions/resources";
import { httpFail, httpOk } from "@/lib/api/response";

export async function GET(req: Request) {
  const url = new URL(req.url);
  const r = await listResources(Object.fromEntries(url.searchParams.entries()));
...
export async function POST(req: Request) {
  ...
  const r = await createResource(body);
  if (!r.ok) {
