-- 145 · notification_kind quedada_payment_reminder + dispatcher branch.
-- El organizador envía un aviso de pago a los inscritos pendientes (paid=false).
-- Se encola desde remindQuedadaPayment (notif inapp + DM del sistema, cooldown 30min).

insert into notification_kinds (kind, description, allowed_roles, default_channels, category) values
  ('quedada_payment_reminder','Recordatorio de pago de una quedada',
   array['user']::mp_role[], array['inapp']::mp_notification_channel[], 'quedadas')
