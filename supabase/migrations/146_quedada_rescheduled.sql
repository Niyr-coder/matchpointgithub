-- 146 · notification_kind quedada_rescheduled + dispatcher branch.
-- El organizador cambia la fecha/hora de la quedada → se avisa a los inscritos.
-- Se encola desde updateQuedadaDetails cuando cambia starts_at.

insert into notification_kinds (kind, description, allowed_roles, default_channels, category) values
  ('quedada_rescheduled','Una quedada en la que estás cambió de fecha',
   array['user']::mp_role[], array['inapp']::mp_notification_channel[], 'social')
