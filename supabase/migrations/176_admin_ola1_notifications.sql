-- 176 · Ola 1 admin: notificaciones críticas de pagos, MP+, broadcast y moderación.
-- Cierra silent failures donde la acción admin mutaba DB pero el usuario no
-- recibía bell ni deep-link operativo.

insert into notification_kinds (kind, description, allowed_roles, default_channels, category) values
  ('broadcast', 'Mensaje masivo enviado por MATCHPOINT',
   array['user']::mp_role[], array['inapp']::mp_notification_channel[], 'marketing'),
  ('payment_captured', 'Pago confirmado por administración',
   array['user']::mp_role[], array['inapp']::mp_notification_channel[], 'pagos'),
  ('mp_plus_activated', 'MATCHPOINT+ activado',
   array['user']::mp_role[], array['inapp']::mp_notification_channel[], 'premium'),
  ('mp_plus_revoked', 'MATCHPOINT+ desactivado',
   array['user']::mp_role[], array['inapp']::mp_notification_channel[], 'premium'),
  ('refund_completed', 'Reembolso registrado',
   array['user']::mp_role[], array['inapp']::mp_notification_channel[], 'pagos'),
  ('report_resolved', 'Reporte revisado por moderación',
   array['user']::mp_role[], array['inapp']::mp_notification_channel[], 'moderation'),
  ('role_assigned', 'Rol asignado',
   array['admin','partner','user','owner','manager','coach','employee']::mp_role[],
   array['inapp']::mp_notification_channel[], 'roles'),
  ('role_revoked', 'Rol revocado',
   array['admin','partner','user','owner','manager','coach','employee']::mp_role[],
   array['inapp']::mp_notification_channel[], 'roles'),
  ('club_staff_assigned', 'Personal agregado a un club',
   array['manager','coach','employee']::mp_role[],
   array['inapp']::mp_notification_channel[], 'roles'),
  ('club_staff_removed', 'Personal removido de un club',
   array['manager','coach','employee']::mp_role[],
   array['inapp']::mp_notification_channel[], 'roles'),
  ('welcome_owner', 'Bienvenida al portal del club',
   array['owner']::mp_role[], array['inapp']::mp_notification_channel[], 'clubs')
