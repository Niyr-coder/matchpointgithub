// Cron: cierra sorteos vencidos y ejecuta draws programados (draw_at).
// Auth: Authorization: Bearer ${CRON_SECRET} o ?token=...

import { NextResponse } from "next/server";
import type { NextRequest } from "next/server";
import {
  closeExpiredGiveaways,
  executeGiveawayDraw,
  getGiveawayCreatedBy,
  listDueGiveawayDraws,
} from "@/server/giveaways/execute-draw";

export const dynamic = "force-dynamic";
export const runtime = "nodejs";

function authorize(req: NextRequest): boolean {
  const expected = process.env.CRON_SECRET;
  if (!expected) return false;
  const header = req.headers.get("authorization") ?? "";
  if (header === `Bearer ${expected}`) return true;
  const token = req.nextUrl.searchParams.get("token");
  return Boolean(token && token === expected);
}

async function handle(): Promise<NextResponse> {
  const closed = await closeExpiredGiveaways();
  const dueIds = await listDueGiveawayDraws(20);
  const drawn: string[] = [];
  const skipped: string[] = [];
  const errors: { id: string; message: string }[] = [];

  for (const id of dueIds) {
    try {
      const actor = (await getGiveawayCreatedBy(id)) ?? id;
      const res = await executeGiveawayDraw(id, actor);
      if (res.skipped) {
        skipped.push(id);
      } else {
        drawn.push(id);
      }
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      errors.push({ id, message });
    }
  }

  return NextResponse.json({
    ok: true,
    data: { closed, drawn, skipped, errors },
  });
}

export async function GET(req: NextRequest) {
  if (!authorize(req)) {
    return NextResponse.json(
      { ok: false, error: { code: "AUTH.UNAUTHORIZED", message: "Token inválido o ausente." } },
      { status: 401 },
    );
  }
  return handle();
}

export async function POST(req: NextRequest) {
  if (!authorize(req)) {
    return NextResponse.json(
      { ok: false, error: { code: "AUTH.UNAUTHORIZED", message: "Token inválido o ausente." } },
      { status: 401 },
    );
  }
  return handle();
}
