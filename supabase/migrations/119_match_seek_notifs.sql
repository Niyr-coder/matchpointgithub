-- 119 · notification_kinds + dispatcher para "Busco partido".
-- Ver docs/guides/02-notifications.md §2 y §4.
--
--  · match_seek_applied   → al AUTOR del aviso, cuando alguien se postula.
--  · match_seek_accepted  → al POSTULANTE, cuando el autor lo acepta.
--
-- Recreamos fn_dispatch_inapp_notifications (idempotente) agregando los 2
-- branches nuevos. El cuerpo se mantiene igual al de 079; solo se suman ramas.

insert into notification_kinds (kind, description, allowed_roles, default_channels, category) values
  ('match_seek_applied',
   'Alguien se postuló a tu "Busco partido"',
   array['user']::mp_role[],
   array['inapp']::mp_notification_channel[],
   'matches'),
  ('match_seek_accepted',
   'Te aceptaron en un "Busco partido"',
   array['user']::mp_role[],
   array['inapp']::mp_notification_channel[],
   'matches')
  ...
  loop
    begin
      ...
      if _kind = 'event_rescheduled' then
        ...
        -- Llevamos directo al chat del partido si tenemos el match.
