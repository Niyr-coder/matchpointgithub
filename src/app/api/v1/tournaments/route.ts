import { createTournament, listTournaments } from "@/server/actions/tournaments";
import { httpFail, httpOk } from "@/lib/api/response";

export async function GET(req: Request) {
  const url = new URL(req.url);
  const r = await listTournaments(Object.fromEntries(url.searchParams.entries()));
...
export async function POST(req: Request) {
  ...
  const r = await createTournament(body);
  if (!r.ok) {
    const c = r.error.code;
    const status =
      c === "AUTH.ROLE_REQUIRED" ? 403
      : c === "TOURNAMENTS.SLUG_TAKEN" ? 409
