    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ overflowX: "auto", paddingBottom: 2 }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(6, minmax(0, 1fr))", gap: 12, minWidth: 0 }}>
          <Kpi label="Marcas" value={String(data.totals.sponsors)} hint={`${data.totals.activeSponsors} activas`} icon="handshake" />
          <Kpi label="Slots" value={String(data.totals.slots)} hint="inventario configurado" icon="layout-grid" />
          <Kpi label="Placements live" value={String(data.totals.activePlacements)} hint="activos ahora" icon="radio-tower" />
          <Kpi label="Contratado" value={money(data.totals.bookedAmountCents)} hint="placements no archivados" icon="wallet" />
          <Kpi label="Impresiones 30d" value={data.totals.impressions30d.toLocaleString("en-US")} hint="eventos reales" icon="eye" />
          <Kpi label="CTR 30d" value={ctr(data.totals.clicks30d, data.totals.impressions30d)} hint={`${data.totals.clicks30d} clics`} icon="mouse-pointer-click" />
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 18 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start", flexWrap: "wrap", marginBottom: 12 }}>
          <div>
            <div className="label-mp" style={{ color: "var(--primary)" }}>● Rate card nativo</div>
          ...
          <span style={{ fontSize: 11, color: "var(--muted-fg)", maxWidth: 360, lineHeight: 1.45 }}>
            Sin ads programáticos ni SDK de publicidad. Cada paquete se registra como slot + placement con monto contratado manual.
          </span>
        </div>
      ...
      {tab === "placements" && (
        <div className="mp-admin-sponsors-grid" style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 360px", gap: 16, alignItems: "start" }}>
          <div className="card" style={{ padding: 0, overflow: "hidden" }}>
            <SectionHead title="Placements vendidos" sub="Campañas por slot, estado y ventana de publicación." />
            {data.placements.length === 0 ? (
              ...
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                  <thead><tr style={{ background: "var(--muted)" }}><Th>Placement</Th><Th>Slot</Th><Th>Estado</Th><Th align="right">30d</Th><Th align="right">Contrato</Th></tr></thead>
                  <tbody>
          ...
          <EditorCard title={placementForm.id ? "Editar placement" : "Nuevo placement"} icon="radio-tower">
            <Field label="Marca"><select value={placementForm.sponsorId} onChange={(e) => setPlacementForm({ ...placementForm, sponsorId: e.target.value })} style={inputStyle}><option value="">Selecciona marca</option>{data.sponsors.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></Field>
            <Field label="Slot"><select value={placementForm.slotId} onChange={(e) => setPlacementForm({ ...placementForm, slotId: e.target.value })} style={inputStyle}><option value="">Selecciona slot</option>{data.slots.map((s) => <option key={s.id} value={s.id}>{s.label} · {s.key}</option>)}</select></Field>
            <Field label="Titular"><input value={placementForm.headline} onChange={(e) => setPlacementForm({ ...placementForm, headline: e.target.value })} style={inputStyle} /></Field>
            <Field label="Texto"><textarea value={placementForm.body} onChange={(e) => setPlacementForm({ ...placementForm, body: e.target.value })} rows={3} style={inputStyle} /></Field>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <Field label="Inicio"><input type="datetime-local" value={placementForm.startsAt} onChange={(e) => setPlacementForm({ ...placementForm, startsAt: e.target.value })} style={inputStyle} /></Field>
              <Field label="Fin opcional"><input type="datetime-local" value={placementForm.endsAt} onChange={(e) => setPlacementForm({ ...placementForm, endsAt: e.target.value })} style={inputStyle} /></Field>
            </div>
            <Field label="URL destino"><input value={placementForm.targetUrl} onChange={(e) => setPlacementForm({ ...placementForm, targetUrl: e.target.value })} style={inputStyle} placeholder="https://..." /></Field>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <Field label="Monto centavos"><input type="number" min={0} value={placementForm.contractAmountCents} onChange={(e) => setPlacementForm({ ...placementForm, contractAmountCents: e.target.value })} style={inputStyle} /></Field>
              <Field label="Prioridad"><input type="number" value={placementForm.priority} onChange={(e) => setPlacementForm({ ...placementForm, priority: e.target.value })} style={inputStyle} /></Field>
            </div>
            <Field label="Estado"><select value={placementForm.status} onChange={(e) => setPlacementForm({ ...placementForm, status: e.target.value as PlacementForm["status"] })} style={inputStyle}><option value="draft">Borrador</option><option value="active">Activo</option><option value="paused">Pausado</option><option value="archived">Archivado</option></select></Field>
      ...
      {tab === "sponsors" && (
        <div className="mp-admin-sponsors-grid" style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 360px", gap: 16, alignItems: "start" }}>
          <ListCard title="Catálogo de marcas" emptyTitle="Sin marcas" emptyHint="Crea una marca para empezar a vender placements.">
          ...
          <EditorCard title={sponsorForm.id ? "Editar marca" : "Nueva marca"} icon="badge-dollar-sign">
            <Field label="Nombre"><input value={sponsorForm.name} onChange={(e) => setSponsorForm({ ...sponsorForm, name: e.target.value })} style={inputStyle} /></Field>
            <Field label="Slug"><input value={sponsorForm.slug} onChange={(e) => setSponsorForm({ ...sponsorForm, slug: e.target.value })} style={inputStyle} placeholder="se-genera-si-lo-dejas-vacio" /></Field>
            <Field label="Web pública"><input value={sponsorForm.websiteUrl} onChange={(e) => setSponsorForm({ ...sponsorForm, websiteUrl: e.target.value })} style={inputStyle} placeholder="https://..." /></Field>
            <Field label="Logo URL"><input value={sponsorForm.logoUrl} onChange={(e) => setSponsorForm({ ...sponsorForm, logoUrl: e.target.value })} style={inputStyle} placeholder="https://..." /></Field>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <Field label="Color marca"><input value={sponsorForm.brandColor} onChange={(e) => setSponsorForm({ ...sponsorForm, brandColor: e.target.value })} style={inputStyle} placeholder="#111827" /></Field>
              <Field label="Contacto"><input value={sponsorForm.contactName} onChange={(e) => setSponsorForm({ ...sponsorForm, contactName: e.target.value })} style={inputStyle} /></Field>
            </div>
            <Field label="Email contacto"><input value={sponsorForm.contactEmail} onChange={(e) => setSponsorForm({ ...sponsorForm, contactEmail: e.target.value })} style={inputStyle} /></Field>
            <Field label="Notas internas"><textarea value={sponsorForm.notes} onChange={(e) => setSponsorForm({ ...sponsorForm, notes: e.target.value })} rows={3} style={inputStyle} /></Field>
      ...
      {tab === "slots" && (
        <div className="mp-admin-sponsors-grid" style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 360px", gap: 16, alignItems: "start" }}>
          <ListCard title="Inventario de slots" emptyTitle="Sin slots" emptyHint="Crea superficies publicitarias antes de vender placements.">
            {data.slots.map((slot) => (
          ...
          <EditorCard title={slotForm.id ? "Editar slot" : "Nuevo slot"} icon="layout-grid">
            <Field label="Key técnico"><input value={slotForm.key} onChange={(e) => setSlotForm({ ...slotForm, key: e.target.value })} style={inputStyle} placeholder="dashboard_user_home" /></Field>
            <Field label="Superficie"><input value={slotForm.surface} onChange={(e) => setSlotForm({ ...slotForm, surface: e.target.value })} style={inputStyle} placeholder="dashboard" /></Field>
            <Field label="Nombre visible"><input value={slotForm.label} onChange={(e) => setSlotForm({ ...slotForm, label: e.target.value })} style={inputStyle} /></Field>
            <Field label="Descripción"><textarea value={slotForm.description} onChange={(e) => setSlotForm({ ...slotForm, description: e.target.value })} rows={3} style={inputStyle} /></Field>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <Field label="Cupos activos"><input type="number" min={1} value={slotForm.maxActivePlacements} onChange={(e) => setSlotForm({ ...slotForm, maxActivePlacements: e.target.value })} style={inputStyle} /></Field>
              <Field label="Precio base centavos"><input type="number" min={0} value={slotForm.basePriceCents} onChange={(e) => setSlotForm({ ...slotForm, basePriceCents: e.target.value })} style={inputStyle} /></Field>
            </div>
            <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, fontWeight: 800 }}>
              <input type="checkbox" checked={slotForm.isActive} onChange={(e) => setSlotForm({ ...slotForm, isActive: e.target.checked })} /> Slot disponible
            </label>
  ...
  return (
    <div className="card" style={{ padding: "12px 13px", display: "flex", flexDirection: "column", gap: 6, minWidth: 0 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 6 }}>
        <span className="label-mp" style={{ fontSize: 9, letterSpacing: "0.1em", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{label}</span>
...
function SectionHead({ title, sub }: { title: string; sub: string }) {
  return (
    <div style={{ padding: "16px 18px", borderBottom: "1px solid var(--border)" }}>
      <h2 className="font-heading" style={{ margin: 0, fontSize: 17, fontWeight: 900, textTransform: "uppercase" }}>{title}<span className="dot">.</span></h2>
...
function EditorCard({ title, icon, children }: { title: string; icon: string; children: ReactNode }) {
  return (
    <div className="card" style={{ padding: 16, display: "flex", flexDirection: "column", gap: 12, position: "sticky", top: 88 }}>
...
function ListCard({ title, emptyTitle, emptyHint, children }: { title: string; emptyTitle: string; emptyHint: string; children: ReactNode[] }) {
  return (
    <div className="card" style={{ padding: 0, overflow: "hidden" }}>
      <SectionHead title={title} sub="Datos reales del backend de sponsors." />
...
function Field({ label, children }: { label: string; children: ReactNode }) {
  return (
    <label style={{ display: "flex", flexDirection: "column", gap: 5, fontSize: 11, fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--muted-fg)" }}>
      {label}
      {children}
    </label>
  );
...
function Th({ children, align }: { children: ReactNode; align?: "left" | "right" }) {
  return <th style={{ padding: "10px 14px", textAlign: align ?? "left", color: "var(--muted-fg)", fontSize: 9.5, fontWeight: 900, letterSpacing: "0.12em", textTransform: "uppercase", whiteSpace: "nowrap" }}>{children}</th>;
}
