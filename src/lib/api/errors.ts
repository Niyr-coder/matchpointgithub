...
export function toHttpError(err: unknown) {
  if (err instanceof ZodError) {
    ...
    return httpFail(400, "VALIDATION.FAILED", "Invalid input", { fields });
  }
  if (err instanceof AuthError) {
    const status = err.code === "AUTH.UNAUTHENTICATED" ? 401 : 403;
    return httpFail(status, err.code, err.message);
  }
  if (err instanceof MpError) {
    return httpFail(err.status, err.code, err.message, err.fields ? { fields: err.fields } : undefined);
  }
  // unknown
   
  console.error("[api] unexpected error", err);
  return httpFail(500, "INTERNAL.UNEXPECTED", "Something went wrong");
}
