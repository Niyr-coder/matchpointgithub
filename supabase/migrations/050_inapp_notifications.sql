-- 050 · In-app dispatcher para notification_jobs.
--
-- Convierte cada notification_jobs.status='pending' (channel='inapp') en una
-- fila visible en `notifications` (la tabla ya existe — ver 022_notifications.sql).
--
-- Nota de diseño:
--   - La tabla `notifications` ya está definida en migration 022, junto con
--     fn_enqueue_notification(SECURITY DEFINER) que la inserta directamente.
--     Para no romper ese flujo, esta migration NO recrea la tabla:
--       * añade columnas opcionales `link` y `delivered_at`
--       * añade un índice parcial para "no leídas" alineado a (user_id, read_at)
--       * mantiene los índices/RLS existentes.
--   - Algunos kinds (event_rescheduled / tournament_rescheduled / plan_expiring_soon)
--     hoy insertan SOLO en notification_jobs sin pasar por fn_enqueue_notification.
--     El dispatcher cubre ambos casos: si el job trae `notification_id` en payload
--     y existe en `notifications`, solo le añade link/delivered_at; si no existe,
--     crea la fila completa.
--   - Status enum permitido (022): pending, sending, sent, failed, skipped.
--     Usamos 'sent' y 'failed'. La columna `sent_at` existe en notification_jobs.
--   - SECURITY DEFINER + search_path fijado a public por seguridad.

-- 1. Columnas nuevas en notifications.
alter table public.notifications
  add column if not exists link text,
  add column if not exists delivered_at timestamptz;

-- Índice parcial "no leídas" sobre (recipient_user_id, read_at).
-- (022 ya tiene idx_notifications_user_role_unread; este es un complemento
-- por user_id sin filtrar por rol para queries del badge global.)
create index if not exists idx_notifications_user_unread
  on public.notifications (recipient_user_id)
  where read_at is null;

-- 2. Dispatcher.
create or replace function public.fn_dispatch_inapp_notifications()
returns integer
language plpgsql
security definer
set search_path = public
as $$
declare
  _job record;
  _kind text;
  _payload jsonb;
  _title text;
  _body text;
  _link text;
  _notif_id uuid;
  _existing_notif uuid;
  _processed int := 0;
  _err text;
begin
  for _job in
    select id, user_id, role, kind, payload
      from public.notification_jobs
     where status = 'pending'
       and channel = 'inapp'
     order by scheduled_for asc
     limit 500
     for update skip locked
  loop
    begin
      _kind := _job.kind;
      _payload := coalesce(_job.payload, '{}'::jsonb);
      _title := nullif(_payload->>'title', '');
      _body := nullif(_payload->>'body', '');
      _link := null;
      _notif_id := null;

      -- Derivación por kind.
      if _kind = 'event_rescheduled' then
        if _title is null then _title := 'Tu evento cambió de fecha'; end if;
        if _body is null then
          _body := coalesce(_payload->>'event_name', 'Un evento')
                || ' fue reprogramado.';
        end if;
        if _payload ? 'event_id' then
          _link := '/eventos/' || (_payload->>'event_id');
        end if;

      elsif _kind = 'tournament_rescheduled' then
        if _title is null then _title := 'Tu torneo cambió de fecha'; end if;
        if _body is null then
          _body := coalesce(_payload->>'tournament_name', _payload->>'event_name', 'Un torneo')
                || ' fue reprogramado.';
        end if;
        if _payload ? 'tournament_id' then
          _link := '/eventos/' || (_payload->>'tournament_id');
        elsif _payload ? 'event_id' then
          _link := '/eventos/' || (_payload->>'event_id');
        end if;

      elsif _kind = 'plan_expiring_soon' then
        if _title is null then _title := 'Tu plan Premium expira pronto'; end if;
        if _body is null then
          if _payload ? 'days_remaining' then
            _body := 'Vence en ' || (_payload->>'days_remaining') || ' días.';
          else
            _body := 'Tu plan Premium está por vencer.';
          end if;
        end if;
        _link := '/dashboard/user/mi-plan';

      elsif _kind = 'reservation_created' then
        if _title is null then _title := 'Reserva confirmada'; end if;
        if _body is null then
          _body := trim(both ' · ' from
            concat_ws(' · ',
              _payload->>'club_name',
              _payload->>'court_name',
              _payload->>'starts_at'
            )
          );
          if _body = '' then _body := 'Tu reserva fue confirmada.'; end if;
        end if;
        _link := '/dashboard/user/reservas';

      else
        -- Fallback: kind desconocido. title = kind, body = payload serializado.
        if _title is null then _title := _kind; end if;
        if _body is null then _body := _payload::text; end if;
        _link := null;
      end if;

      -- Si el payload trae notification_id y existe, solo enriquecemos.
      if _payload ? 'notification_id' then
        begin
          _existing_notif := (_payload->>'notification_id')::uuid;
        exception when others then
          _existing_notif := null;
        end;
      end if;

      if _existing_notif is not null then
        update public.notifications
           set link = coalesce(link, _link),
               delivered_at = coalesce(delivered_at, now())
         where id = _existing_notif;
        if found then
          _notif_id := _existing_notif;
        end if;
      end if;

      -- Si no había notification previa, la creamos.
      if _notif_id is null then
        insert into public.notifications
          (recipient_user_id, recipient_role, kind, title, body, payload, link, delivered_at)
        values
          (_job.user_id, _job.role, _kind, _title, _body, _payload, _link, now())
        returning id into _notif_id;
      end if;

      update public.notification_jobs
         set status = 'sent',
             sent_at = now(),
             attempts = attempts + 1
       where id = _job.id;

      _processed := _processed + 1;

    exception when others then
      _err := SQLERRM;
      update public.notification_jobs
         set status = 'failed',
             attempts = attempts + 1,
             last_error = _err
       where id = _job.id;
    end;
  end loop;

  return _processed;
end;
$$;

revoke all on function public.fn_dispatch_inapp_notifications() from public;
grant execute on function public.fn_dispatch_inapp_notifications() to service_role;

-- 3. Cron cada 5 minutos.
do $$
begin
  if exists (select 1 from cron.job where jobname = 'dispatch-inapp-notifs') then
    perform cron.unschedule('dispatch-inapp-notifs');
  end if;
  perform cron.schedule(
    'dispatch-inapp-notifs',
    '*/5 * * * *',
    $cron$ select public.fn_dispatch_inapp_notifications() $cron$
  );
end $$;
