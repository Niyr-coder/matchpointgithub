-- 122 · notification_kinds + dispatcher para ciclo de vida de matches.
--  · match_cancelled   → al otro participante cuando se cancela el partido.
--  · match_rescheduled → al otro participante cuando cambia la fecha/hora.
-- Recrea fn_dispatch_inapp_notifications agregando los 2 branches. El link
-- va al chat del partido (conversation_id en payload).

insert into notification_kinds (kind, description, allowed_roles, default_channels, category) values
  ('match_cancelled',
   'Tu partido fue cancelado',
   array['user']::mp_role[],
   array['inapp']::mp_notification_channel[],
   'matches'),
  ('match_rescheduled',
   'Tu partido cambió de fecha/hora',
   array['user']::mp_role[],
   array['inapp']::mp_notification_channel[],
   'matches')
  ...
  loop
    begin
      ...
      if _kind = 'event_rescheduled' then
        ...
        if _body is null then
          _body := coalesce(_payload->>'applicant_name', 'Un jugador')
                || ' se postuló a tu "Busco partido". Revisa y acepta para jugar.';
        end if;
        _link := '/dashboard/user/busco-partido';
        if _payload ? 'seek_id' then
          _link := _link || '?focus=' || (_payload->>'seek_id');
        end if;
      elsif _kind = 'match_seek_accepted' then
        if _title is null then _title := 'Te aceptaron el partido'; end if;
        if _body is null then
          _body := coalesce(_payload->>'author_name', 'El autor')
                || ' aceptó tu postulación. Coordinen por el chat del partido.';
        end if;
        if _payload ? 'conversation_id' then
          _link := '/dashboard/user/chat?conv=' || (_payload->>'conversation_id');
        else
          _link := '/dashboard/user/busco-partido';
        end if;
        ...
        if _body is null then
          _body := coalesce(_payload->>'canceller_name', 'El otro jugador')
                || ' canceló el partido'
                || coalesce(' · ' || nullif(_payload->>'reason', ''), '') || '.';
        end if;
        if _payload ? 'conversation_id' then
          _link := '/dashboard/user/chat?conv=' || (_payload->>'conversation_id');
        end if;
        ...
        if _body is null then
          _body := coalesce(_payload->>'rescheduler_name', 'El otro jugador')
                || ' propuso una nueva fecha para el partido. Revisa el chat.';
        end if;
        if _payload ? 'conversation_id' then
          _link := '/dashboard/user/chat?conv=' || (_payload->>'conversation_id');
        end if;
