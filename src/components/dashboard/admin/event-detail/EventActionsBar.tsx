...
// Barra lateral de acciones admin del detalle de evento. Autocontenida:
// maneja su propio estado de diálogos y llama server actions. Extension
// point compartido por varios agentes:
// - Agente A (editar/reprogramar): añade botón "Editar"
// - Agente C (refunds): no toca aquí (acciones por-fila en TransactionsTable)
// - Agente D (organizador): añade "Contactar" y "Reasignar organizador"

import { useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import { Icon } from "@/components/Icon";
import { cancelEvent, type AdminEventDetail } from "@/server/actions/events";
import { reassignEventOrganizerAdmin } from "@/server/actions/admin-event-ownership";
...
export function EventActionsBar({ data }: { data: AdminEventDetail }) {
  ...
  const handleCancel = () => {
    startTransition(async () => {
      ...
      if (res.ok) {
        toast({ icon: "check", title: "Evento cancelado" });
        setCancelOpen(false);
        setReason("");
        router.refresh();
      } else {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      {cancelOpen && (
        <CancelDialog
          title={`Cancelar evento "${data.event.name}"`}
          reason={reason}
          setReason={setReason}
      ...
      {reassignOpen && (
        <ReassignOrganizerDialog
          title={`Reasignar organizador de "${data.event.name}"`}
          currentLabel={organizerLabel}
          onClose={() => setReassignOpen(false)}
          onConfirm={async (userId) => {
            const res = await reassignEventOrganizerAdmin({
              eventId: data.event.id,
...
// ── Diálogo compartido (solo este archivo) ─────────────────────────────────
// Se reusa la server action `searchUsers` ya existente en roles.ts
// (admin/owner/manager pueden buscar; búsqueda por username/display_name).
